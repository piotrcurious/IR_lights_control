// ESP32 LEDC PWM + IR remote (safe: software-managed fades, duty caching + throttling)
...
struct Effect {
  ...
  float dutyCycle = 50.0; // Percent (for pulse)
  uint32_t duration = 1000; // ms for one direction (for fade)
  ...
  uint32_t nextUpdateMs = 0; // throttle next allowed update
...
// Throttle: how often each fade channel updates (ms). 20 ms gives ~50 Hz update.
const uint32_t FADE_STEP_MS = 20;

// Per-channel last duty cache to avoid driver thrash. Initialized to invalid so first write happens.
static uint32_t lastDuty[LEDC_CHANNEL_COUNT];
...
// Cached duty writer: avoid calling LEDC driver when duty didn't change.
static inline void ledcWriteDuty(uint8_t channel, uint32_t duty) {
  if (channel >= LEDC_CHANNEL_COUNT) return;
  uint32_t maxV = maxForRes(currentResolution);
  if (duty > maxV) duty = maxV;
  if (lastDuty[channel] == duty) return;   // cheap path: nothing to do
...
void setup() {
  ...
  Serial.println();
  Serial.println("ESP32 LED Controller — software fades (throttled, cached duties)");
  Serial.println();
 Serial.println();
  Serial.println("======================================================");
  ...
  // Initialize per-channel hardware to stored brightness (ledcWriteDuty will write)
  for (int i = 0; i < LEDC_CHANNEL_COUNT; ++i) {
    ledcWriteDuty(i, brightness[i]);
    yield();
  }

  // Autostart effects stored in preferences - these will use software fades
  for (int i = 0; i < LEDC_CHANNEL_COUNT; i++) {
  ...
  // initial flash to indicate selection
...
void loop() {
  ...
  if (IrReceiver.decode()) {
    if (IrReceiver.decodedIRData.protocol == SAMSUNG &&
      ...
      uint8_t cmd = IrReceiver.decodedIRData.command & 0xFF;
...
void reconfigureLEDCTimer(uint8_t newRes) {
  ...
  for (int ch = 0; ch < LEDC_CHANNEL_COUNT; ++ch) {
    ...
    // scale lastDuty as well so we don't cause a spurious write
...
void setupLEDCDriver() {
  ...
  for (int ch = 0; ch < LEDC_CHANNEL_COUNT; ++ch) {
    ...
    // leave actual duty write to caller (we initialized lastDuty to UINT32_MAX)
    yield();
  }
}
...
void handleDigitSelection(int digit) {
  if (digit < 0 || digit >= LEDC_CHANNEL_COUNT) {
    Serial.printf("Digit %d out of range (only 0..%d supported)\n", digit, LEDC_CHANNEL_COUNT - 1);
    return;
  }
...
void processSamsungCommand(uint32_t rawCmd) {
  ...
  switch (cmd) {
    ...
    case RemoteKeys::KEY_9:
      Serial.printf("Digits 8/9 unsupported (only 0..%d channels available)\n", LEDC_CHANNEL_COUNT - 1);
      startFlash(selectedIndex);
      break;
    ...
    case RemoteKeys::KEY_DOWN:
      if (isChaining && sourceChainChannel != -1) {
        ...
        if (cmd == RemoteKeys::KEY_UP) nextTypeInt++; else nextTypeInt--;
        if (nextTypeInt > (int)EFFECT_FADE_IN_OUT) nextTypeInt = (int)EFFECT_PULSE;
        if (nextTypeInt < (int)EFFECT_PULSE) nextTypeInt = (int)EFFECT_FADE_IN_OUT;
        ...
        if (cmd == RemoteKeys::KEY_UP) reconfigureLEDCTimer(currentResolution + 1);
        else reconfigureLEDCTimer(currentResolution - 1);
      }
      break;
    ...
    case RemoteKeys::KEY_RIGHT:
    {
      ...
      if (cmd == RemoteKeys::KEY_LEFT) e.frequency /= FREQ_STEP_RATIO; else e.frequency *= FREQ_STEP_RATIO;
      ...
      Serial.printf("Channel %d frequency changed to %.3f Hz\n", targetChannel, e.frequency);
    }
    break;
...
// -------------------- Effects (software fades, throttled) --------------------
...
void applyFrequencyChangeToChannel(uint8_t channel) {
  ...
  if (e.type == EFFECT_PULSE) {
    ...
    if (e.frequency <= 0.0f) e.frequency = 1.0f;
    float period = 1000.0f / e.frequency;
    e.duration = (uint32_t)(period / 2.0f + 0.5f); // ms per direction
    e.lastStepTime = millis();
    e.nextUpdateMs = millis(); // allow immediate update
...
void startEffect(uint8_t channel, EffectType type, bool oneShot) {
  ...
  effects[channel].nextUpdateMs = millis();
  ...
  if (type == EFFECT_FADE_IN_OUT) {
    ...
    // Start from zero brightness; software fade will step up
    ledcWriteDuty(channel, 0);
    Serial.printf("Started FADE (soft) on ch %u: freq=%.3fHz duration=%ums\n", channel, effects[channel].frequency, (unsigned)effects[channel].duration);
...
void updateEffects() {
  ...
  for (int i = 0; i < LEDC_CHANNEL_COUNT; ++i) {
    ...
    if (effects[i].type == EFFECT_FADE_IN_OUT) {
      // Throttle updates per channel
      if ((int32_t)(now - effects[i].nextUpdateMs) < 0) {
        // not time yet for this channel
        continue;
      }
      effects[i].nextUpdateMs = now + FADE_STEP_MS;

      uint32_t elapsed = now - effects[i].lastStepTime;
      uint32_t dur = effects[i].duration;
      if (dur == 0) dur = 1;
      ...
      if (t >= 1.0f) {
        if (effects[i].oneShot) {
          ...
          // toggle direction and reset timer
          effects[i].lastStepTime = now;
          if (effects[i].state == STATE_FADE_UP) {
            effects[i].state = STATE_FADE_DOWN;
            ledcWriteDuty(i, effects[i].savedBrightness); // endpoint
          } else {
        ...
        uint32_t target = effects[i].savedBrightness;
        uint32_t duty;
        if (effects[i].state == STATE_FADE_UP) {
          duty = (uint32_t)((float)target * t + 0.5f);
        } else {
          duty = (uint32_t)((float)target * (1.0f - t) + 0.5f);
        }
        ledcWriteDuty(i, duty); // cached writer will skip if unchanged
      }
    } else if (effects[i].type == EFFECT_PULSE) {
      if (effects[i].state == STATE_PULSE_ON && now - effects[i].lastStepTime > effects[i].onTime) {
        if (effects[i].oneShot) {
          effectFinished = true;
        } else {
          effects[i].lastStepTime = now;
      ...
      } else if (effects[i].state == STATE_PULSE_OFF && now - effects[i].lastStepTime > effects[i].offTime) {
        effects[i].lastStepTime = now;
...
void startFlash(uint8_t channel) {
  ...
  flashState.prevWasFading = (effects[channel].type == EFFECT_FADE_IN_OUT);
  if (flashState.prevWasFading) {
    // Stop software fades by setting state idle; duty will be restored later
    effects[channel].state = STATE_IDLE;
  }
...
void updateSaveIndicator() {
  ...
  if ((int32_t)(millis() - saveIndicatorState.nextToggleMs) >= 0) {
    ...
    if (saveIndicatorState.blinksRemaining > 0) {
      ...
      for (int ch = 0; ch < LEDC_CHANNEL_COUNT; ++ch) ledcWriteDuty(ch, duty);
      saveIndicatorState.nextToggleMs = millis() + toggleInterval;
    } else {
      for (int i = 0; i < LEDC_CHANNEL_COUNT; ++i) {
        ledcWriteDuty(i, brightness[i]);
        if (wasFading[i]) startEffect(i, EFFECT_FADE_IN_OUT, false);
        yield();
      }
...
void indicateSave() {
  for (int i = 0; i < LEDC_CHANNEL_COUNT; ++i) {
    ...
    if (wasFading[i]) {
      effects[i].state = STATE_IDLE; // stop software fade
    }
    yield();
  }
