...
struct Effect {
  ...
  float frequency = 1.0; // Hz (user-adjustable)
  float dutyCycle = 50.0; // Percent (for pulse)
  uint32_t duration = 1000; // ms for one direction (for fade). kept for compatibility
...
void processSamsungCommand(uint32_t rawCmd) {
  ...
  switch (cmd) {
    ...
    {
      // Change frequency of the effect either for sourceChainChannel (if chaining)
      // or for currently selected channel.
      ...
      // If there is no effect yet, still allow adjusting the stored frequency
      if (e.frequency <= 0.0f) e.frequency = 1.0f;

      if (cmd == RemoteKeys::KEY_LEFT) {
        e.frequency /= FREQ_STEP_RATIO;
      } else {
        e.frequency *= FREQ_STEP_RATIO;
      }
      ...
      // Apply change immediately if effect running
...
void startEffect(uint8_t channel, EffectType type, bool oneShot) {
  ...
  // ensure effect frequency is sane
  ...
  if (type == EFFECT_FADE_IN_OUT) {
    ...
    // compute duration from frequency: duration is ms for one direction (period/2)
...
void stopEffect(uint8_t channel) {
  ...
  // If the effect is a hardware fade, we must stop it to prevent
  // a race condition where the fade interrupt conflicts with new commands.
...
void startFlash(uint8_t channel) {
  ...
  // Gracefully interrupt any hardware fade on the target channel to prevent conflicts.
  flashState.wasFading = (effects[channel].type == EFFECT_FADE_IN_OUT);
  if (flashState.wasFading) {
    // Set the duty to the current brightness to stop the fade without disabling the channel.
    ledcWriteDuty(channel, brightness[channel]);
...
void indicateSave() {
  // Gracefully interrupt any hardware fades to prevent conflicts with the save animation
  for (int i = 0; i < LEDC_CHANNEL_COUNT; ++i) {
